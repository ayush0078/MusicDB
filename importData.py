...
#Read the csv file to load the CIty nodes
    query = """
    LOAD CSV WITH HEADERS FROM 'file:///neoData.csv' AS s 
...
#Creating nodes for all artists from a csv file
    query = """
    LOAD CSV WITH HEADERS FROM 'file:///neoData.csv' AS s MERGE (a:artists {a_name:s.Artist})
...
#Creating nodes for all Playlists from a csv file
    query = """
    LOAD CSV WITH HEADERS FROM 'file:///neoData.csv' AS s MERGE (p:playlists {p_name:s.Playlist})
...
#Creating relation between the artists and the tracks
    query = """
    LOAD CSV WITH HEADERS FROM 'file:///neoData.csv' AS s MERGE (t:tracks{t_name:s.Song,r_year:toInteger(s.Year),rating:toInteger(s.Rating),genre:s.Genre,album_name:s.Album,duration:s.Duration})
